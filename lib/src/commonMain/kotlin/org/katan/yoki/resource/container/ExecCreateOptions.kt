 ...
 * - [attachStdin] `-i` or `--interactive`;
 * - [attachStdout] and [attachStderr] Not available;
 * - [detachKeys] `--detach-keys`;
 * - [tty] `-t` or `--tty`;
 * - [env] `-e` or `--env`;
 * - [privileged] `--privileged`;
 * - [user] `-u` or `--user`;
 * - [workingDir] `-w` or `--workdir`
 *
 * @property attachStdin Should attach standard input (stdin) on exec creation.
 * @property attachStdout Should attach standard output (stdin) on exec creation.
